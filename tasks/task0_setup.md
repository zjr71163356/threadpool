# Task 0: 环境准备与多线程基础

**预计时间：** 30-60 分钟
**难度：** ⭐ 入门

---

## 📖 学习目标

完成本任务后，你将：
- ✅ 配置好 C++11 开发环境
- ✅ 理解线程的基本概念
- ✅ 掌握 `std::thread` 的基本用法
- ✅ 理解 join 和 detach 的区别
- ✅ 了解数据竞争的概念

---

## 🎯 任务内容

### 1. 环境配置

#### 检查编译器版本
```bash
g++ --version  # 需要 GCC 4.8+ 或 Clang 3.3+
```

#### 编译测试
```bash
cd /home/tyrfly1001/threadpool
g++ -std=c++11 -pthread tasks/task0_example.cpp -o task0_test
./task0_test
```

### 2. 理解 std::thread 基础

#### 2.1 创建和启动线程

```cpp
#include <iostream>
#include <thread>

void hello() {
    std::cout << "Hello from thread!" << std::endl;
}

int main() {
    std::thread t(hello);  // 创建并启动线程
    t.join();              // 等待线程完成
    return 0;
}
```

**关键点：**
- `std::thread` 构造时线程就开始运行
- 必须调用 `join()` 或 `detach()`，否则析构时会调用 `std::terminate()`

#### 2.2 传递参数

```cpp
void print_sum(int a, int b) {
    std::cout << a << " + " << b << " = " << (a + b) << std::endl;
}

int main() {
    std::thread t(print_sum, 10, 20);
    t.join();
    return 0;
}
```

#### 2.3 使用 Lambda

```cpp
int main() {
    int x = 100;
    std::thread t([x]() {
        std::cout << "x = " << x << std::endl;
    });
    t.join();
    return 0;
}
```

#### 2.4 join vs detach

**join():**
- 阻塞当前线程，等待目标线程完成
- 线程结束后可以安全访问线程的结果
- 类似于 POSIX 的 `pthread_join()`

**detach():**
- 分离线程，让它在后台独立运行
- 主线程不再等待该线程
- 必须确保线程访问的数据在线程结束前有效
- 类似于 POSIX 的 `pthread_detach()`

```cpp
// 使用 join
void example_join() {
    std::vector<int> data = {1, 2, 3};
    std::thread t([&data]() {
        for (int i : data) {
            std::cout << i << " ";
        }
    });
    t.join();  // 等待线程完成，data 仍然有效
}

// 使用 detach（危险！）
void example_detach_dangerous() {
    std::vector<int> data = {1, 2, 3};
    std::thread t([&data]() {  // ⚠️ 引用捕获
        for (int i : data) {
            std::cout << i << " ";
        }
    });
    t.detach();  // ❌ data 可能在线程运行前就被销毁
}
```

### 3. 数据竞争与互斥锁

#### 3.1 数据竞争示例

```cpp
#include <iostream>
#include <thread>
#include <vector>

int counter = 0;  // 共享变量

void increment() {
    for (int i = 0; i < 100000; ++i) {
        ++counter;  // ⚠️ 数据竞争！
    }
}

int main() {
    std::vector<std::thread> threads;
    for (int i = 0; i < 10; ++i) {
        threads.emplace_back(increment);
    }

    for (auto& t : threads) {
        t.join();
    }

    // 期望值：1,000,000
    // 实际值：通常小于 1,000,000
    std::cout << "Counter: " << counter << std::endl;
    return 0;
}
```

**为什么会出错？**
- `++counter` 不是原子操作
- 实际包含：读取、加1、写回
- 多个线程同时操作会导致数据竞争

#### 3.2 使用 mutex 保护

```cpp
#include <mutex>

std::mutex mtx;
int counter = 0;

void increment_safe() {
    for (int i = 0; i < 100000; ++i) {
        std::lock_guard<std::mutex> lock(mtx);  // RAII 风格加锁
        ++counter;  // ✅ 受保护
    }
}
```

---

## ✅ 实践练习

### 练习 1：创建多个线程

编写程序创建 5 个线程，每个线程打印自己的 ID 和一条消息。

**期望输出：**
```
Thread 0: Hello
Thread 1: Hello
Thread 2: Hello
Thread 3: Hello
Thread 4: Hello
```

**提示：** 使用 `std::this_thread::get_id()`

### 练习 2：参数传递

编写程序，创建线程计算两个数的和，并在主线程中打印结果。

**要求：**
- 使用共享变量存储结果
- 使用 mutex 保护共享变量
- 主线程等待计算完成后打印结果

### 练习 3：理解数据竞争

运行上面的数据竞争示例（3.1），多次执行，观察结果的变化。然后：
1. 解释为什么结果每次不同
2. 修复数据竞争问题
3. 验证修复后的结果正确

---

## 🤔 思考题

1. **为什么必须调用 join() 或 detach()？**
   - 如果不调用会发生什么？
   在 std::thread 对象的析构函数执行时，如果该线程仍然是 joinable 的（即没被 join 也没被 detach）
   - 程序会正常退出吗？
   不会

2. **什么时候使用 join，什么时候使用 detach？**
   - 各自的适用场景是什么？
   一个是
   - detach 有什么风险？

3. **为什么数据竞争会导致错误的结果？**
   - `++counter` 在汇编层面是什么操作？
   - 画出时序图说明两个线程如何产生竞争

4. **使用 mutex 后性能如何？**
   - 与不加锁相比慢多少？
   - 为什么？

---

## 📚 参考资料

### C++ 多线程基础
- [cppreference: std::thread](https://en.cppreference.com/w/cpp/thread/thread)
- [cppreference: std::mutex](https://en.cppreference.com/w/cpp/thread/mutex)
- [C++ Concurrency in Action - Chapter 2](https://www.manning.com/books/c-plus-plus-concurrency-in-action-second-edition)

### 数据竞争与同步
- [C++ Memory Model](https://en.cppreference.com/w/cpp/language/memory_model)
- [Race Condition vs Data Race](https://blog.regehr.org/archives/490)

---

## ✅ 检查清单

完成本任务前，请确认：

- [ ] 已安装支持 C++11 的编译器
- [ ] 成功编译并运行示例代码
- [ ] 理解 join 和 detach 的区别
- [ ] 能够创建线程并传递参数
- [ ] 理解数据竞争的概念
- [ ] 知道如何使用 mutex 保护共享数据
- [ ] 完成所有练习题
- [ ] 思考并回答思考题

---

## 下一步

完成本任务后，继续 [Task 1: 线程包装类](task1_thread_wrapper.md)

在 Task 1 中，你将学习如何封装线程，为后续的线程池实现打下基础。
